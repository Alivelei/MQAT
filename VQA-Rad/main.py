...
def compute_img_norm(configure):
    img_queries = os.listdir(configure["dataset"]["images_path"])
    img_path = [os.path.join(configure['dataset']["images_path"], query) for query in img_queries]
...
class Sort2Id(object):
    ...
    def building_classify(self):
        ...
        locations = set([query["image_organ"] for query in queries])
