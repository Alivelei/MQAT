...
#     if qus:
#         queries = sentence.lower().strip("?").strip(" ").strip(")").strip("(").split(" ")  # 将问题进行切分，且都转换为小写
#     else:
...
def sentence_to_word(sentence, qus=True):
    if qus:
        ...
        queries = sentence.split()  # 将每个单词进行切分
        queries = " ".join(queries)  # 转换为句子
...
class SaveWord2Vec(object):
    ...
    def forward(self):
        ...
        for query in queries:
            questions.append(sentence_to_word(query["question"], True))  # 1229
            answers.append(sentence_to_word(query["answer"], False))  # 476
        ...
        self.ans_ws.build_vocab()  # qus:1229  ans:476
