...
def compute_batch_score(ans_pred, ans, ans_type):
    ...
    # 计算open、close的相等的数量
    organ_batch_acc = (organ_ans_pred == organ_ans).sum()
    modality_batch_acc = (modality_ans_pred == modality_ans).sum()
    plane_batch_acc = (plane_ans_pred == plane_ans).sum()
    abnormality_batch_acc = (abnormality_ans_pred == abnormality_ans).sum()
