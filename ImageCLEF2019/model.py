...
class ImgPatchEmbedding(nn.Module):
    def __init__(self, emb_size=784, seq_len=49, ngf_conv=64):
        ...
        self.feature_extractor = nn.Sequential(
            ...
            Rearrange('b e (h) (w) -> b (h w) e')  # [batch, 256, 7, 7] --> [batch, 256, 7*7] --> [batch, 49, 256]
        )

        self.positions = nn.Parameter(torch.randn(seq_len, emb_size))  # 加入位置编码

    def forward(self, x):
        x = self.feature_extractor(x)  # torch.Size([2, 49, 768])
...
class TextPatchEmbedding(nn.Module):
    ...
    def forward(self, qus):
        text_embedding = self.embedding(qus)  # [batch, seq_len, 300]

        text_x = self.linear(text_embedding)  # [batch, seq_len, 768]
...
class VILT(nn.Module):
    def __init__(self, configure):
        ...
        qus_word_size = configure["config"]["qus_word_size"]  # 270
        n_classes = configure["config"]["ans_word_size"]
        emb_size = configure["model"]["emb_size"]
        qus_embedding_dim = configure["model"]["qus_embedding_dim"]
        text_seq_len = configure["config"]["qus_seq_len"]
    ...
    def forward(self, img, qus):
        ...
        patch_embedding = torch.cat([patch_embedding, img_patch_features], dim=1)  # [batch, 70, 768]
...
if __name__ == '__main__':
    ...
    print(vit(a, b).shape)
    print(sum(x.numel() for x in vit.parameters()))
